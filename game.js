...
function animate(f,dt){
  ...
  if(f.parts.rightArm.attached){let rx=-.08+.12*walk,rz=.13,ry=0;if(f.weaponHeld){if(f.weapon==='spear'){rz=.98;rx=-.04;ry=.03;if(f.action==='attack'){rz=1.18;rx=-.02;ry=.02;} }else{rz=.28;rx=-.12;if(f.action==='attack'){rz=.4+1.0*as;rx=-.25-.45*as;ry=-.2*as;}}if(f.action==='parry'){rz=.55+1.05*ps;rx=-.12-.25*ps;ry=.28*ps;}}else if(f.attackKind==='punch'&&f.action==='attack')rz=1.05*as;f.pivots.rightArm.rotation.set(rx,ry,rz);}
  if(f.weaponObj&&f.weaponHeld){f.weaponObj.position.y=-.02;f.weaponObj.position.z=0;if(f.weapon==='spear'){const thrust=f.action==='attack'?Math.sin(Math.min(1,ap)*Math.PI)*.78:0;f.weaponObj.position.x=.08+thrust;f.weaponObj.rotation.set(0,0,0);}else{f.weaponObj.position.x=.24;f.weaponObj.rotation.set(0,0,f.action==='parry'?-.45*ps:0);}}
